...
def reset_terminal():
    ...
    if result["success"]:
        while True:
            token = input_quit_handle("Enter the reset token from your email: ")
            ...
            if resetpass != new_password:
                typing_effect(red + "Passwords do not match. Please try again." + reset)
                continue
            ...
            if reset_result["success"]:
                typing_effect(
                    green + "Password reset successfully! You can now log in." + reset
                )
                sleep()
                clear()
                return
